...
public class DateUtils {
    ...
    public static Date getDayStart(Date date) {
        String str = DateUtils.formatDate(date,DATE_PATTERN);
        str = str+" 00:00:00";
        return DateUtils.convertDate(str,DATETIME_PATTERN);
    }
    ...
    public static String getCurrentTimestampStr() {
        Timestamp ts = new Timestamp(getTodayDate().getTime());
        String tsStr = ts.toString().replace("-", "").replace(" ", "").replace(":", "");
        return tsStr.substring(0, tsStr.length() - 2);
    }
